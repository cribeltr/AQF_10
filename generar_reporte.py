...
<style>
  ...
  *{box-sizing:border-box}
  ...
  .nota-cell{max-width:340px;white-space:normal;line-height:1.35}
  .nota-empty{color:var(--muted);font-style:italic}
  tr.has-nota td.nota-cell{box-shadow:inset 3px 0 0 var(--brand);background:#f3f8ff}
  textarea.note-edit{width:100%;min-height:170px;padding:9px;border:1px solid var(--line);
    border-radius:8px;font:inherit;font-size:13px;resize:vertical}
  .note-meta{font-size:11.5px;color:var(--muted);margin-top:6px}
  .modal label{display:block;font-weight:600;color:var(--brand-dark);margin:8px 0 4px}
...
<header class="top">
  ...
  <p>Hospital Hernán Henríquez Aravena · listado único de equipos con notas + eventos · importable y cliqueable</p>
</header>
...
function setData(d){
  DATA=d;
  EQUIPOS=DATA.equipos.filter(e=>e.serie||e.inventario);   // listado único (equipo real)
  EQ_BY_KEY={}; EQUIPOS.forEach(e=>{ EQ_BY_KEY[equipKey(e)]=e; });
}
...
$("csv").addEventListener("click",()=>{
  ...
  if(TAB==="equipos"){
    ...
    lines.push(EVENT_COLS.map(c=>c[1]).join(";"));
